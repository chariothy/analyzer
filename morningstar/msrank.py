...
1. 分离出天天基金的基金经理到另一个爬虫
    '''
    sql = f'''
SELECT b.code AS code, b.name AS fund_name, c.name AS cat_name, c.banchmark AS banchmark, b.class_id AS class_id, b.reg_date AS reg_date
, round(json_value(b.fee, '$.Management')+json_value(b.fee, '$.Custodial'),2) AS fee
, r.y2_risk_rating AS y2riskr, r.y3_risk_rating AS y3riskr, r.y5_risk_rating AS y5riskr, r.y10_risk_rating AS y10riskr
, r.y3_ms_rating AS y3msr, r.y5_ms_rating AS y5msr, r.y10_ms_rating AS y10msr
, r.y3_ms_risk AS y3risk, r.y5_ms_risk AS y5risk, r.y3_std AS y3std, r.y5_std AS y5std, r.y3_sharp AS y3sharp, r.y5_sharp AS y5sharp
, r.alpha_ind AS alphai, r.beta_ind AS betai, r.r2_ind AS r2i, r.alpha_cat AS alphac, r.beta_cat AS betac, r.r2_cat AS r2c
...
, e.ytd_return AS ytdr, e.ytd_cat_rank AS ytdcr, e.ytd_cat_size AS ytdcs
, e.y1_return AS y1r, e.y1_cat_rank AS y1cr, e.y1_cat_size AS y1cs
, e.y2_return AS y2r, e.y2_cat_rank AS y2cr, e.y2_cat_size AS y2cs
, e.y3_return AS y3r, e.y3_cat_rank AS y3cr, e.y3_cat_size AS y3cs
, e.y5_return AS y5r, e.y5_cat_rank AS y5cr, e.y5_cat_size AS y5cs, e.return_date AS return_date
, e.asset AS asset, p.cash_p AS cash_p, p.stock_p AS stock_p, p.bond_p AS bond_p, p.other_p AS other_p, p.top_stock_w AS top_stockw
, p.portfolio_date AS portfolio_date, m.managers
...
FROM spd_ms_fund_base b, json_table(b.manager, '$[*]' columns(
   ...
   is_leave boolean path '$.Leave')) as jt
WHERE jt.is_leave=0 GROUP BY b.code) AS m
WHERE b.category_id=c.class_id AND b.CODE=r.CODE AND b.CODE=e.CODE AND b.CODE=p.CODE and b.code=m.code
...
1。 return cat_rank/cat_size
    ...
    del df['class_id']
    df.to_csv(f'{folder}/fund-{pu.today()}.csv', encoding='utf_8_sig')
    return df
